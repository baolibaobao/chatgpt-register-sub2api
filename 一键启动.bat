...
REM Before running, api_code mother emails are split into Gmail plus variants once.
REM Env options:
REM   SPLIT_MOTHER_COUNT=4     number of variants per mother email
...
if exist "scripts\split_mother_email.py" (
    if /I not "%SKIP_SPLIT_MOTHER%"=="1" (
        ...
        echo [INFO] Mother lines will be removed after splitting to avoid duplicate splitting.
        ".venv\Scripts\python.exe" "scripts\split_mother_email.py" --count !SPLIT_MOTHER_COUNT! --remove-mother
        if errorlevel 1 (
